...
def _записать_итоги(session: Session, вид: str, результаты: list[dict]) -> None:
    for р in результаты:
        ...
        if р.get("status") == "accepted":
            ...
            continue
        ошибка = (р.get("error", "") or "")
        # Отказ проведения УТ отдаёт с trace_id и НЕ журналирует его у себя —
        # причина может быть транзиентной (блокировка, ещё не синхронизованная
        # ссылка). Такой документ НЕ фиксируем: повторим на следующем прогоне,
        # УТ по client_uid дубль не создаст. Деловой отказ (наши понятные
        # тексты про заполнение/данные) — постоянный, фиксируем, чтобы не
        # долбить УТ каждым прогоном; разбирается вручную по ut_exports.error.
        if "trace_id" in ошибка:
            log.warning("%s %s: отказ проведения, повтор — %s", вид, cuid, ошибка)
            continue
        session.add(UtExport(client_uid=cuid, kind=вид, error=ошибка[:1000]))
    session.commit()
