...
Поля, которые ведутся только в SmartSale (блокировка клиента, штрихкод,
кратность упаковки), при обновлении НЕ трогаем: их источник — кабинет, а не УТ,
и обмен не должен их затирать. Кредитный контроль (лимит, срок оплаты, долг,
просрочка) — наоборот, мастер в УТ: его зеркалим при каждом обмене.
...
def _int(значение) -> int:
    """Целое из строки УТ. Пустое/битое → 0."""
    if значение in (None, ""):
        return 0
    try:
        return int(Decimal(str(значение)))
    except (InvalidOperation, ValueError):
        return 0
...
# Порог «плохого» долга по «Стандарту работы ТП»: Рабочий — без просрочки,
# Плохой — просрочка больше этого порога дней (нет оплат >90 дней), между —
# Проблемный. Стандарт называет Проблемный «≤60 дней», а Плохой «>90»; диапазон
# 61..90 стандартом не назван — относим его к Проблемному (ещё не Плохой).
# «Плохой» по стандарту требует ещё и отсутствия обновлённого графика погашения —
# на Фазе 1 это не отслеживаем, статус только по дням.
_ПОРОГ_ПЛОХОГО_ДОЛГА = 90


def _статус_долга(дней_просрочки: int) -> str:
    """working / problem / bad по числу дней просрочки."""
    if дней_просрочки <= 0:
        return "working"
    if дней_просрочки > _ПОРОГ_ПЛОХОГО_ДОЛГА:
        return "bad"
    return "problem"
...
def _принять_клиентов(session: Session) -> None:
    ...
    for э in ut_client.получить_страницами("customers"):
        ...
        к.active = bool(э.get("active", True))
        # Кредитный контроль — мастер в действующем договоре УТ: лимит, срок
        # оплаты, флаги и текущий долг/просрочку зеркалим при каждом обмене.
        # payment_type и blocked остаются локальными (кабинет SmartSale).
        к.credit_limit = _dec(э.get("credit_limit"))
        к.deferral_days = _int(э.get("deferral_days"))
        к.limit_enabled = bool(э.get("limit_enabled", False))
        к.forbid_overdue = bool(э.get("forbid_overdue", False))
        к.debt = _dec(э.get("debt"))
        к.overdue_debt = _dec(э.get("overdue_debt"))
        к.overdue_days = _int(э.get("overdue_days"))
        к.debt_status = _статус_долга(к.overdue_days)
        if к.id is None:
            session.add(к)
            клиенты[э["uid"]] = к
